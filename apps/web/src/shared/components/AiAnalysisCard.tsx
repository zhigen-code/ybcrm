  ...
  const executeAction = async (analysis: AiAnalysis, action: AiAction) => {
    ...
    try {
      ...
      const ACTION_DESCRIPTIONS: Record<string, string> = {
        update_lead_status:       `${t('ai.analysis.suggestions.updateLeadStatus', { value: action.value })} ${action.reason}`,
        update_contract_status:   `${t('ai.analysis.suggestions.updateContractStatus', { value: action.value })} ${action.reason}`,
        update_intended_services: `${t('ai.analysis.suggestions.updateService', { value: action.value })} ${action.reason}`,
        update_service_plans:     `${t('ai.analysis.suggestions.updatePlan', { value: action.value })} ${action.reason}`,
        reassign_sales:           `${t('ai.analysis.suggestions.reassign', { value: action.value })} ${action.reason}`,
      }
      ...
      if (action.type === 'create_activity') {
        ...
        await crmApi.post('/activities', {
          ...base,
          description: `${t('ai.analysis.suggestions.nextContact', { value: action.value })} ${action.reason}`,
      ...
      alert(msg ? `${t('ai.analysis.opFailed')}${msg}` : t('ai.analysis.opError'))
    }
  }
    ...
    <div className="rounded-xl border bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b bg-gradient-to-r from-violet-50 to-white">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-800">{t('ai.analysis.title')}</span>
          {latest && (
        ...
        >
          {isAnalyzing ? t('ai.analysis.analyzing') : latest ? t('ai.analysis.regenerate') : t('ai.analysis.start')}
        </Button>
      </div>

      {analyzeMutation.isError && (
        <div className="px-4 py-3 bg-red-50 text-sm text-red-600">
          {(analyzeMutation.error as { response?: { data?: { message?: string } } })?.response?.data?.message ?? t('ai.analysis.error')}
        </div>
      )}
        ...
        <>
          ...
          {actions.length > 0 && (
            <div className="px-4 py-3">
              <p className="text-xs font-semibold text-gray-500 mb-2">{t('ai.analysis.suggestions')}</p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-1 mb-1">
                        ...
                        {isDone && (
                          <span className="text-xs text-green-600 flex-shrink-0">✓ {t('ai.analysis.executed')}</span>
                        )}
                      </div>
